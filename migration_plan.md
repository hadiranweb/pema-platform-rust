# طرح مهاجرت سرویس‌های PEMA به Rust

این سند طرح مهاجرت سرویس‌های پلتفرم PEMA از زیرساخت فعلی (که شامل Docker بود) به پیاده‌سازی بومی Rust را تشریح می‌کند. هدف اصلی، بازنویسی هر سرویس به Rust و اطمینان از عملکرد صحیح آن بدون نیاز به کانتینرسازی است. همچنین، توجه ویژه‌ای به پیاده‌سازی صحیح قابلیت صفحه‌بندی (Pagination) در هر سرویس خواهد شد.
