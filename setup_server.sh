...
DB_NAME="pema_db"
BACKEND_PORT_API="8081"
BACKEND_PORT_AUTH="8082"
...
sudo certbot --nginx -d ${DOMAIN} -d www.${DOMAIN} --non-interactive --agree-tos --email ${CERTBOT_EMAIL} || log_error "Certbot failed to obtain certificates. Check DNS and Nginx config."
...
server {
    ...
    location /api/ {
        proxy_pass http://127.0.0.1:${BACKEND_PORT_API};
    ...
    location /auth/ {
        proxy_pass http://127.0.0.1:${BACKEND_PORT_AUTH};
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto \$scheme;
...
log_info "Configuring .env files..."
